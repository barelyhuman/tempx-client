...
const documentationLink =
  'https://documenter.getpostman.com/view/10646880/SzRxUprP?version=latest';
...
export default class Home extends React.Component {
  ...
  uploadFileOnDrop(e) {
    ...
    e.forEach((fileToUpload) => {
      ...
      axios
        .post(`${config.APIURL}/files/upload?expiry=30 secs`, fd, {
        ...
        .then((data) => {
          const { fileKeys } = this.state;
          this.setState({
            fileKeys: { ...fileKeys, [uploadFileKey]: data.data.accessKey },
          });
        })
        .catch(() => {});
    });
    ...
    return (
      <main className="main-container">
        <Head>
          ...
          />
          <link
            href="https://fonts.googleapis.com/css2?family=Abel&family=Ubuntu&display=swap"
            rel="stylesheet"
          />
        ...
        <nav className="flex just-space-between align-baseline flex-wrap">
          <div className="margin-x-2 margin-y-2">
            <a className="animated-underline title" href="/">
              TempX |{' '}
              <span>
                <small>Temporary File Storage</small>
              </span>
            </a>
          </div>
        ...
        <section>
          ...
          {currentFileList.length ? (
            <div className="file-list-container width-100">
                        ...
                        {fileKeys[item.name] ? (
                          <>
                            <div className="flex align-center">
                              <button
                                type="button"
                                className="link-button animated-underline"
                                onClick={() => this.openImage(item.name)}
                              >
                                {item.name}
                              </button>
                              ...
                              <Spacer x={1} inline/>
                              <span
                                onClick={() => this.copyLink(item.name)}
                              >
                                <i className="gg-copy" />
                              </span>
                            </div>
              ...
              <Button
                className="block margin-x-auto flat-button"
                onClick={() => this.setState({ currentFileList: [] })}
              >
                Upload More
              </Button>
            </div>
        ...
        <footer className="sticky-footer flex just-center align-center">
          <button
            ...
            type="button"
            className="margin-x-2 dark-mode-button link-button"
